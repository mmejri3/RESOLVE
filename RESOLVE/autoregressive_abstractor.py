...
'''class AutoregressiveHDFormer(tf.keras.Model):
    ...
    def __init__(self,
        ...
        # initialize decoder
        #self.decoder = MultiAttentionDecoder(**decoder_kwargs, name='decoder')
        self.mha = HDSymoblicAttention(1024,100)
        # initialize final prediction layer
        self.final_layer = layers.Dense(output_dim, name='final_layer')
        self.hidden_layer = layers.Dense(1024, name='hidden_layer')
        self.bn1 = layers.LayerNormalization(epsilon=1e-7,epsilon=1e-7)
        self.bn2 = layers.LayerNormalization(epsilon=1e-7,epsilon=1e-7)
        self.dropout = layers.Dropout(0.2)
        dropout_rate = 0.2
        self.dropout0 = layers.Dropout(dropout_rate, name='dropout0')
        ...
        self.layernorm = [layers.LayerNormalization(epsilon=1e-7,epsilon=1e-6, scale=False,name=f'layer_norm_{i}') for i in range(3)]  
        self.bias_s = self.add_weight(shape=(36, 1024), initializer='glorot_normal', trainable=True, name='bias_s')
        self.bias_t = self.add_weight(shape=(36, 1024), initializer='glorot_normal', trainable=True, name='bias_t') 
        self.W_Q = self.add_weight(shape=(64, 1024), initializer='glorot_normal', trainable=True, name='w_q')
        #self.W_K = self.add_weight(shape=(64, 1024), initializer='glorot_normal', trainable=False, name='w_k')
        #self.W_V = self.add_weight(shape=(64, 1024), initializer='glorot_normal', trainable=False, name='w_v')         
        self.layerNorm_t = layers.LayerNormalization(epsilon=1e-7,epsilon=1e-7, name='layer_norm_t')
        self.layerNorm_s = layers.LayerNormalization(epsilon=1e-7,epsilon=1e-7, name='layer_norm_s')
        self.encoder = Encoder(**encoder_kwargs, name='encoder')
        self.word_to_id = word_to_id           
        self.end_token = self.word_to_id(';')
        self.start_token = self.word_to_id('@') 
    ...
    def cosine_similarity(self, a, b):
        b_n = tf.math.sign(b)
        a_n = tf.math.sign(a)
        score = tf.reduce_sum(a_n * b_n, axis=-1) / b_n.shape[-1] 
        return tf.nn.softmax(score,axis=-1)
                
    def cross_attention(self, T, h_O, causality=True, bind=True):
        T_Q, T_K, T_V = T[0], T[1], T[2]
        b_T_Q = h_O[:, tf.newaxis, :, :] 
        b_T_K = T_K[:, :, tf.newaxis, :]   
        cross_att_scores = self.cosine_similarity(b_T_Q, b_T_K)  
        if causality: 
            shape = tf.shape(cross_att_scores)
            B = shape[0]
            N = shape[1]
            M = shape[2]
            mask = tf.linalg.band_part(tf.ones((N, M)), -1, 0)
            cross_att_scores = tf.einsum('bik,ik->bik', cross_att_scores, mask)
        output = tf.einsum('bij,bki->bkj', h_O, cross_att_scores) 
        if bind:
            output = tf.nn.relu(output* self.bias_s)
        output = self.layerNorm_s(output) 
        output = self.dropout(output)      
        return output
       
    
    def self_attention(self, T, causality=True, trained_scores = True):
        T_Q, T_K, T_V = T[0], T[1], T[2]
        b_Q = T_Q[:, tf.newaxis, :, :]  
        b_K = T_K[:, :, tf.newaxis, :] + T_K[:, tf.newaxis, :, :]   
        cross_att_scores = self.cosine_similarity(b_K, b_Q) 
        if causality: 
            shape = tf.shape(cross_att_scores)
            B = shape[0]
            N = shape[1]
            M = shape[2]    
            i = tf.range(M)[:, tf.newaxis] 
            j = tf.range(N)      
            mask = tf.linalg.band_part(tf.ones((N, M)), -1, 0)      
            cross_att_scores = tf.einsum('bik,ik->bik', cross_att_scores, mask) 
        output = tf.einsum('bij,bki->bkj', T_V, cross_att_scores) 
        output = tf.nn.tanh(output* self.bias_t) 
        output = self.layerNorm_t(output) 
        output = self.dropout(output)      
        return output
    
    
    def process_target(self, h_t):
        O = tf.einsum('bij,ij->bij',h_t,self.X)
        O = tf.cumsum(O,axis=1,exclusive=False)
        return O
    def decoder(self, T, h_O, h_A, state=None):
        h_T = self.self_attention(T, causality=True)
        T = [h_T, h_T, h_T]
        attn_output = self.cross_attention(T, h_O, causality=False)
        x = (h_T + attn_output)/2
        x = self.layernorm[0](x)
        x = self.dropout1(x)
        attn_output = self.cross_attention([x,x,x], h_A, causality=False)
        x = (h_T + attn_output)/2
    ...
    def call(self, inputs):
        ...
        source = self.source_embedder(source)
        source = self.pos_embedding_adder_input(source)
        h_A, h_O = self.mha(source)
        h_A = self.dropout(h_A)
        h_A = self.bn2(h_A)
        target = self.target_embedder(target)
        target = self.pos_embedding_adder_target(target)
        T_Q = tf.einsum('bij,jk->bik',target,self.W_Q)
        T_Q = self.dropout(T_Q)
        T = [T_Q, T_Q, T_Q]
        x = self.decoder(T, h_O, h_A)
    ...
    def get_initial_state(self, context, max_length):
        ...
        zeros_tensor = tf.zeros([batch_size, max_length - 2], dtype=tf.int64)
        start_target = tf.concat([start_tokens, zeros_tensor], axis=1)        
        return start_target, done
    ...
    def translate(self,
        ...
        for iteration in range(max_length - 1):
            if max_length - iteration - 3 > 0:
                next_token, done = self.get_next_token(context, next_target, iteration, done)
                zeros_tokens = tf.zeros([batch_size, max_length - iteration - 3], dtype=tf.int64)
                next_target = tf.concat([next_target[:, :iteration + 1], next_token], axis=1)
                next_target = tf.concat([next_target, zeros_tokens], axis=1)
            tokens.append(next_token)
...
class AddPositionalEmbedding2(tf.keras.layers.Layer):
    def __init__(self, max_length=1024, name="add_positional_embedding2"):
        super().__init__(name=name)
...
class SmallConvNet(tf.keras.Model):
    def __init__(self):
        ...
        self.conv1 = layers.Conv1D(filters=64, kernel_size=5, strides=1, padding='same')
        self.conv2 = layers.Conv1D(filters=1024, kernel_size=5, strides=3, padding='same')
        self.add = layers.Add()
...
class Binding(tf.keras.Model):
    def __init__(self,
        ...
        self.encoder = Encoder(**encoder_kwargs, name='encoder')
        self.hd_encoder = AnotherSmallConvNet()
    ...
    def call(self, inputs):
        ...
        target_hd = self.hd_encoder(target_hd)
        target_hd = self.bn_t(target_hd)
...
class AutoregressiveHDFormer(tf.keras.Model):                
    def __init__(self,
        ...
        self.down = layers.Dense(64, activation='linear')
        self.decoder = MultiAttentionDecoder(**decoder_kwargs, name='decoder')
    ...
    def call(self, inputs):
        h_t, h_e, h_a = self.encoder(inputs)
        h_t_d = self.down(h_t)
        h_e_d = self.down(h_e)
        h_a_d = self.down(h_a)  
