...
# Symbolic abstracter
    ...
    if args.pretraining_mode == 'none':
        def create_model(seed):
            ...
            #tf.config.experimental.enable_op_determinism()
            model = seq2seq_abstracter_models.Seq2SeqSymbolicHDFormer(**hdformer_kwargs)
            model.compile(loss=loss, optimizer=create_opt(), metrics=metrics)                      
            #print(argsort_model.summary())
            return model          
    ...
    if args.pretraining_mode == 'none':
        def create_model(seed):
            tf.config.experimental.enable_op_determinism()
            model = seq2seq_abstracter_models.Seq2SeqCorelNet(**hdformer_kwargs)
            model.compile(loss=loss, optimizer=create_opt(), metrics=metrics)                      
            #print(argsort_model.summary())
            return model          
