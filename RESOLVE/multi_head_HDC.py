...
class MultiHeadAttention(Layer):
    ...
    def _build_from_signature(self, query, value, key=None):
        ...
        with tf_utils.maybe_init_scope(self):
            ...
            self.Weights = layers.Dense(4,activation='softmax')
            self._query_dense = [layers.Dense(self._key_dim,activation='relu',trainable=True) for _ in range(4)]
            einsum_equation, bias_axes, output_rank = _build_proj_equation(
    ...
    def _compute_attention(
        ...
        # attention scores.
        attention_scores = self.Weights(query)
    ...
    def call(
        ...
        #   H = `size_per_head`
        # `query` = [B, T, N ,H]
        query = [tf.expand_dims(self._query_dense[i](query[:, i, :]),1) for i in range(len(self._query_dense))]
